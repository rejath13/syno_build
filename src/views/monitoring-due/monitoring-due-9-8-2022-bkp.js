...
function DynamicTable({ columns, data, fromNumber, toNumber, getSubscriptionsList, totalCount, activeCustomerCount, showReminderPopup, dueremindercount }) {
    ...
    return (
        <>
            <Form>
                <Form.Row>
                    <Col xs={2} style={{ color: 'black' }}><span style={{ top: '12px', position: 'relative' }}><b>Total active customers : {activeCustomerCount}</b></span></Col>
                    <Col xs={2}>
...
function App() {
    ...
    const getReminders = useCallback(async () => {
        ...
        try {
            ...
            const url = API_URL + "getMonitoringDueReminders";

            const response = await fetch(url, options)

            const data = await response.json();

            setDueReminderCount(data.data.length);
    ...
    const updateMonitoringNote = (selectedcustomerdetails) => {
        ...
        if (selectedcustomerdetails.monitoring_due_note) {
            setNoteArray(selectedcustomerdetails.monitoring_due_note);
        } else {
            setNoteArray([]);
        }
    }
        ...
        () => [
                ...
                Cell: ({ row }) => {
                    ...
                    return (
                        <span onClick={() => update(row.original)} style={{ cursor: 'pointer' }}>
                            <span style={{ color: 'black' }}><b>{row.original.name.replace(/&amp;/g, '&')}</b></span>
            ...
            },
            {
                ...
                Cell: ({ row }) => {
                    ...
                    return (
                        <span>{row.original.customer_account_monitored_time && row.original.customer_account_monitored_time !== '0000-00-00' ? row.original.customer_account_monitored_time : ''}</span>
                    );
                ...
                Cell: ({ row }) => {
                    ...
                    return (
                        <span>
                            ...
                            >
                                <span style={{ fontSize: '12px', marginLeft: '2px' }}>{row.original.monitoring_due_reminder_date && row.original.monitoring_due_reminder_date !== '0000-00-00 00:00:00' ? '(' + Moment(row.original.monitoring_due_reminder_date).format('YYYY-MM-DD HH:mm') + ')' : ''}</span>
                            </OverlayTrigger>
    ...
    const getSubscriptionsList = useCallback(async ({ pageIndex, searchText, sortBy, listType }) => {
        ...
        const postdata = {
            keyword: searchText, sortType: stype ? stype : sortType, sortOrder: sorder ? sorder : sortOrder, listType: listType?'monitored':'due'
        }
    ...
    const saveMonitoringNote = async (data) => {
        ...
        if (getValues('customer_account_monitored')===false || (getValues('customer_account_monitored')===true && getValues('monitoring_note'))) {
            ...
            const postdata = { customer_id: selectedCustomerId, monitoring_note: data.monitoring_note.replace(/\r?\n/g, '<br/>'), monitoring_date: monitoringDate ? Moment(monitoringDate).format('YYYY-MM-DD HH:mm:ss') : '', monitoring_note_id: data.monitoring_note_id, customer_account_monitored:data.customer_account_monitored===true?'yes':'no' };
                                                    ...
                                                    <li className="media">
                                                        ...
                                                        <div className="media-body">
                                                            <h6 className="media-heading text-muted">{note.monitoring_note_by}
                                                                ...
                                                                <span style={{ fontWeight: 'bold', marginRight: '25px', float: 'right', color: 'black' }}>{note.monitoring_note_date ? '[' + note.monitoring_note_date + ']' : ''}</span>
                                    ...
                                    <Table responsive hover style={{ border: '1px solid #eaeaea' }}>
                                        ...
                                        <tbody>
                                            {reminders.map((reminder, index) => (
                                                <tr>
                                                    <td>{reminder.subscription_due_priority && reminder.subscription_due_priority === true && <i className="fas fa-star" style={{ color: 'red' }}></i>}</td>
                                                    <td>{index + 1}</td>
                                                    <td>{reminder.name}</td>
                                                    <td>{reminder.count+reminder.Noconnection}</td>
                                                    <td><span className={findDatePastorFuture(reminder.monitoring_due_reminder_date)<0?'redText':''}>{Moment(reminder.monitoring_due_reminder_date).format('DD-MM-YYYY hh:mm:ss')}</span></td>
                                                    <td>{reminder.monitoring_note_text}</td>
                                                    <td>
