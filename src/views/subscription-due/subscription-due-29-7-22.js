...
function App() {
    ...
    const getReminders = useCallback(async (rectifiedFilter,selectedRemType) => {
        ...
        try {
            ...
            const url = `${API_URL}getSubscriptionDueReminders/${selectedRemType}/${rectifiedFilter}`;
    ...
    useEffect(() => {
        getReminders(rectifiedFilter,selectedRemType);
    }, [rectifiedFilter,selectedRemType])

    const filterRectifiedReminders = (e) =>{
        let filter = e.target.checked;
        setRectifiedFilter(filter);
    }
    ...
    const updateMonitoringNote = (selectedcustomerdetails) => {
        ...
        reset1({
            ...
            subscription_note_rectified: false,
            subscription_note_by:'',
            subscription_note_assigned_to:loginUserName
        });
        setMonitoringDate();

        if (selectedcustomerdetails.subscription_due_note) {
            setNoteArray(selectedcustomerdetails.subscription_due_note);
        } else {
            setNoteArray([]);
        }
    }
    ...
    const saveMonitoringNote = async (data) => {
        ...
        const postdata = { ...data, customer_id: selectedCustomerId, monitoring_note: data.monitoring_note.replace(/\r?\n/g, '<br/>'), monitoring_date: monitoringDate ? Moment(monitoringDate).format('YYYY-MM-DD HH:mm') : '',subscription_note_rectified:data.subscription_note_rectified};
    ...
    const fillformtoedit = (note) => {
        reset1({
            ...
            subscription_note_rectified: note.subscription_note_rectified==="false"?false:true,
    ...
    const handleRectifiedStatusChange = (e) => {
        reset1({...getValues(),subscription_note_rectified:e.target.checked});
    }
        ...
        <React.Fragment>
                                ...
                                <Card.Body className='task-comment'>
                                                    ...
                                                    <li className="media">
                                                        ...
                                                        <div className="media-body">
                                                            <h6 className="media-heading text-muted">{note.subscription_note_by}
                                                                ...
                                                                <span style={{ fontWeight: 'bold', marginRight: '25px', float: 'right', color: 'red' }}>{note.subscription_note_date!=='0000-00-00 00:00:00' ? '[' + note.subscription_note_date + ']' : ''}</span> : <span style={{ fontWeight: 'bold', marginRight: '25px', float: 'right', color: 'black' }}>{note.subscription_note_date!=='0000-00-00 00:00:00' ? '[' + note.subscription_note_date + ']' : ''}</span>}

                                                            </h6>
                                                            <p style={{ color: 'black' }}>{ReactHtmlParser(note.subscription_note_text)} </p>
                                    ...
                                    <Form key="monitoringform" onSubmit={handleSubmit1(saveMonitoringNote)}>
                                        ...
                                        <Row>
                                            ...
                                            <Col md="2">
                                                {getValues('subscription_note_assigned_to')===loginUserName && <Form.Group>
                                                    <div className="checkbox d-inline">
                                                      <Form.Control type="checkbox" name="rectified" value="false" checked={getValues('subscription_note_rectified')} id="rectified" onChange={handleRectifiedStatusChange} />
                                                      <Form.Label htmlFor="rectified" className="cr m-0">Rectified </Form.Label>
                                                    </div>
                                                </Form.Group>}  
                                            </Col>
                                        </Row>
            ...
            <Modal size="xl" show={reminderPopup} onHide={() => setReminderPopup(false)}>
                <Modal.Header closeButton>
                    ...
                    <Row style={{margin:'0 auto',width:'40%'}}>
                        <Col md="4">
                            {selectedRemType==='others'&&<div className="checkbox d-inline">
                                <Form.Control type="checkbox" name="rectifiedfilter" value={rectifiedFilter} id="rectifiedfilter" onChange={filterRectifiedReminders} />
                                <Form.Label htmlFor="rectifiedfilter" className="cr m-0">Rectified </Form.Label>
                            </div>}
                        </Col>
                        <Col md="8">
                                    ...
                                    <Table responsive hover style={{ border: '1px solid #eaeaea', maxHeight:'80vh', overflowY: 'scroll' }}>
                                        ...
                                        <tbody>
                                            {reminders.map((reminder, index) => (
                                                <tr>
                                                    <td>
                                                        <img className="left-logo" style={{borderRadius:'50%',width:'20px'}} src={reminder.subscription_note_by==='admin'?adminprofile:reminder.subscription_note_by==='Shams'?shamsprofile:reminder.subscription_note_by==='Shamnad'?shamnadprofile:reminder.subscription_note_by==='Rasick'?rasickprofile:reminder.subscription_note_by==='Ajmal'?ajmalprofile:reminder.subscription_note_by==='Celine'?celineprofile:reminder.subscription_note_by==='Shone'?shoneprofile:adminprofile} alt="Generic placeholder" />
                                                        {reminder.subscription_note_rectified ==='true' && <OverlayTrigger placement='top' overlay={<Tooltip id={`tooltip-top`}>Rectified</Tooltip>}>
                                                                <i className="fa fa-check" style={{ color: 'green', marginLeft:'10px'}}></i>
                                                            </OverlayTrigger>}

                                                        {reminder.subscription_due_priority && reminder.subscription_due_priority === true && <i className="fas fa-star" style={{ color: 'red',float:'right',marginTop:'5px' }}></i>}
                                                    </td>
                                                    <td>{index + 1}</td>
                                                    <td>{reminder.name}</td>
                                                    {selectedRemType!=='self'&&<td>{reminder.subscription_note_assigned_to===loginUserName?'Self':reminder.subscription_note_assigned_to}</td>}
                                                    <td><span className={getDifferenceInDays(reminder.subscription_note_date)?'redText':''}>{Moment(reminder.subscription_note_date).format('DD-MM-YYYY hh:mm:ss')}</span></td>
                                                    <td>{reminder.subscription_note_text}</td>
                                                    <td>
