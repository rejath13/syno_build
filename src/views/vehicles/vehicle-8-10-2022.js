...
function DynamicTable({ columns, data, fromNumber, toNumber, getVehiclesList, totalCount, addVehicle, viewReminderPopup, fnremainder, dueCount, TodayCount, upcomingCount, RectCount, dueremCount
}) {
    ...
    const {
    ...
    } = useTable(
        {
            ...
            data,
            manualSortBy: true,
            initialState: { pageIndex: 0, sortBy: [{ id: 'uniqueid', desc: false }] },
    ...
    return (
        <>
            <Form>
                <Form.Row>
                    <Col xs={1} style={{ color: 'black' }}><span style={{ top: '12px', position: 'relative' }}><b>Total : {totalCount}</b></span></Col>
                    <Col xs={2}>
                        <Form.Group>
                            ...
                            <Form.Label>
                                ...
                                >
                                    <span>Reminders {dueremCount > 0 && '(' + dueremCount + ')'}</span>
                                </OverlayTrigger>
                    ...
                    <Col xs={1}>
                        {loginUserId === '1' ? <form action={`${API_URL}VehiclesReport/${sortBy[0].id}/${sortBy[0].desc ? 'desc' : 'asc'}/${rmode}/${selectedRemType}/${rectifiedFilter}/${remSearchDate}/${searchReminder}/${searchtext}/${selectedStatus}`} method="get" target="_blank">
...
function App() {
    ...
    const [dueremCount, setDueremCount] = useState(null);
    ...
    const [sortType, setSortType] = useState('uniqueid');
    ...
    const updateMNote = (reminderdata) => {
        setselectedVehicleid(reminderdata.id);
    ...
    const showReminderPopup = async (vehicle) => {
        ...
        try {
            ...
            const url = API_URL + "getVehicleNotes/" + vehicle.id;
        ...
        () => [
            ...
            },
            {
                ...
                Cell: ({ row }) => {
                    ...
                    return (
                        <span>
                            {row.original.name}<br />{row.original.customer_name} ({row.original.username})<br /><OverlayTrigger placement='top' overlay={<Tooltip id={`tooltip-top`}>Created on</Tooltip>}><span style={{ fontSize: '11px' }}>{Moment(row.original.created_at).format('DD-MM-YYYY HH:mm')}</span></OverlayTrigger>
                        </span>
                    );
        ...
        () => [

            {
                ...
                Cell: ({ row }) => {
                    ...
                    return (
                        <span>
                            <span className="pointer" onClick={() => getVehicle(row.original.id)}>{row.original.name}</span>
                            &nbsp;
                            <OverlayTrigger placement='top' overlay={<Tooltip id={`tooltip-top`}>Account List</Tooltip>}>
                                <i className="fa fa-eye pointer" onClick={() => getVehicleAccountsList(row.original.id)} style={{ fontWeight: 'normal', margin: 0 }}></i>
                            </OverlayTrigger>
                            &nbsp;&nbsp;
                            ...
                            {checkDeviceNC(row.original.lastupdate) ?
                                <span>
                                    <OverlayTrigger
                                        placement='top'
            ...
            },
            {
                ...
                Cell: ({ row }) => {
                    ...
                    return (<span>{row.original.customer_name} ({row.original.username})
                        <br />
                        <OverlayTrigger
                            placement='top'
                            overlay={<Tooltip id={`tooltip-top`}>Customer Status</Tooltip>}
                        >
                            <span className={row.original.customer_status==='blocked'?'redtext':'greentext'} style={{ 'text-transform': 'capitalize' }}>{row.original.customer_status}</span>
                        </OverlayTrigger>
                    </span>);
            ...
            },
            {
                ...
                Cell: ({ row }) => {
                    ...
                    return (
                        <span><span>{row.original.uniqueid}
                            <OverlayTrigger
                                placement='top'
                                overlay={<Tooltip id={`tooltip-top`}>Sim</Tooltip>}>
                                <span><br />{row.original.phone}</span>
                            </OverlayTrigger>
...

            {
                ...
                Cell: ({ row }) => {
                    return (<span>{Moment(row.original.lastupdate).format('DD-MM-yyyy hh:mm a')}</span>);
                }
            },
            {
                Header: 'Network',
                ...
                Cell: ({ row }) => {
                    ...
                    return (
                        <span>
                            ...
                            <a className='btn btn-primary text-capitalize' onClick={() => loginToWeb(row.original.username, row.original.password)} href='#' style={{ padding: '6px' }}>
                                <i className="fas fa-sign-in-alt" style={{ margin: 0 }}></i>
                            </a>

                            {row.original.vehicle_blocked == 'yes' ?
                                <Button onClick={() => { fnblocked('comment', row.original.id); }} className='text-capitalize fa fa-sticky-note' variant="primary" style={{ padding: '6px' }}></Button> : ''}
    ...
    const getVehiclesList = useCallback(async ({ pageIndex, searchtext, sortBy, selectedStatus, rectifiedFilter, selectedRemType, remSearchDate, rmode, searchReminder }) => {
        ...
        const postdata = {
            keyword: searchtext, sortType: stype ? stype : sortType, sortOrder: sorder ? sorder : sortOrder, status: selectedStatus
        }
        ...
        if (rmode === false || rmode === undefined) {
            ...
            try {
                ...
                setDueremCount(data.dueremindercount);
            ...
            try {
                ...
                setTotalCount(data.dueremindercount);
                setDueremCount(data.duecnt);
                            ...
                            <Col md={12}>
                                <Card style={{ margin: 0 }}>
                                    <Card.Body className='task-comment'>
                                        <span className='accountvehicle'><i class="fas fa-circle"></i> {item.customer_name} ({item.username})</span>
                                    </Card.Body>
                                </Card>
                            </Col>
                        </Row>
                                        ...
                                        <Row>
                                            <Col md={4}>
                                                <Form.Group controlId="exampleForm.ControlSelect1">
                                                    ...
                                                    <input type="text" class="form-control" disabled={isDisabled}  {...register('name')} />
                                            ...
                                            <Col md={2} className="p-0">
                                                <Form.Group controlId="exampleForm.ControlSelect1">
                                                    ...
                                                    <input type="text" class="form-control" disabled={isDisabled} {...register('uniqueid')} />
                                            ...
                                            <Col md={2}>
                                                <Form.Group controlId="exampleForm.ControlSelect1">
                                                    ...
                                                    <input type="text" class="form-control" disabled={isDisabled} {...register('phone')} />
