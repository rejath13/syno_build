        ...
        <>
            <Form>
                <Form.Row>
                    ...
                    {rmode === false &&
                        <Col xs={1}>{loginUserId === '1' || totalCount <= 200 ?
...
function App() {
    ...
    const [rectifiedFilter, setRectifiedFilter] = useState(false);
    const [selectedRemType, setSelectedRemType] = useState('self');
    const [remSearchDtSave, setRemSearchDtSave] = useState('today');
    const [searchReminder, setSearchReminder] = useState(null);
    ...
    const getVehicle = async (vehid, customerid) => {
        ...
        try {
            ...
            const url = API_URL + "singlevehicle/" + vehid + "/" + customerid;
            ...
            setDuplicates(vehicledata.data.inactive === 1 ? true : false);
            setvehicle_blocked(vehicledata.data.vehicle_blocked === 'yes' ? true : false);
            reset(vehicledata.data);

            reset({ ...getValues(), inactive: vehicledata.data.inactive === 1 ? true : false, customer_id: customerid });
    ...
    const editvehicleform = async (data) => {
        ...
        if (data.vehicle_traccar_id != '') {
            ...
            try {
                ...
                const url = API_URL + "editvehicle/" + data.vehicle_traccar_id;
                ...
                Cell: ({ row }) => {
                    ...
                    return (<span style={{ display: 'flex' }}>
                        ...
                        <OverlayTrigger placement='top' overlay={<Tooltip id={`tooltip-top`}>Edit Vehicle</Tooltip>}>
                            <Button onClick={() => getVehicle(row.original.vehicle_note_vehicle_id, row.original.customer_traccar_id)} variant="dark" style={{ padding: '6px' }}>
                                <i class="fa fa-edit" style={{ fontWeight: 'normal', margin: 0 }}></i>
                            </Button>
                        </OverlayTrigger>

                        <OverlayTrigger placement='top' overlay={<Tooltip id={`tooltip-top`}>Account Login</Tooltip>}>
                            <Button onClick={() => loginToWeb(row.original.customer_username, row.original.password)} variant="primary" style={{ padding: '6px' }}>
        ...
        () => [

            {
                ...
                Cell: ({ row }) => {
                    ...
                    return (
                        <span>
                            <span className="pointer" onClick={() => getVehicle(row.original.vehicle_traccar_id, row.original.customer_traccar_id)}>{row.original.vehicle_name}</span>
                            <OverlayTrigger placement='top' overlay={<Tooltip id={`tooltip-top`}>Account List</Tooltip>}>
                                <i className="fa fa-eye pointer" onClick={() => getVehicleAccountsList(row.original.vehicle_traccar_id)} style={{ fontWeight: 'normal', margin: '0 0 0 5px' }}></i>
                            </OverlayTrigger>

                            <br />

                            {row.original.rental_device === 'yes' ? <span className="text-danger mr-2">(Rental)</span> : ''}

                            <OverlayTrigger
                                placement='top'
                                overlay={<Tooltip id={`tooltip-top`}>Device Type</Tooltip>}>
                                <Badge variant="warning">{row.original.vehicle_device_type}</Badge>
                            </OverlayTrigger>

                            {(row.original.vehicle_blocked === 'yes' || row.original.customer_status === 'blocked') ?
                                <OverlayTrigger placement='top'
                                    overlay={<Tooltip id={`tooltip-top`}>Blocked</Tooltip>}>
                                    <Badge variant="danger">B</Badge>
                                </OverlayTrigger> : ''}
                            ...
                            {row.original.vehicle_ownership === 'customer' ? <OverlayTrigger placement='top'
                                overlay={<Tooltip id={`tooltip-top`}>Customer Device</Tooltip>}>
                                <Badge variant="success">CD</Badge>
                            </OverlayTrigger> : ''}
            ...
            },
            {
                ...
                Cell: ({ row }) => {
                    ...
                    return (<span className="pointer" onClick={() => getVehicle(row.original.vehicle_traccar_id, row.original.customer_traccar_id)}>{row.original.customer_name} ({row.original.customer_username})
                        <br />
                        <OverlayTrigger
                            placement='top'
                            overlay={<Tooltip id={`tooltip-top`}>Customer Status</Tooltip>}
                        >
                            <span className={row.original.customer_status === 'blocked' ? 'text-danger mr-2' : 'text-success mr-2'} style={{ 'text-transform': 'capitalize' }}>{row.original.customer_status === 'followup' ? 'Newly Created' : row.original.customer_status}</span>
                        </OverlayTrigger>
                        <OverlayTrigger placement='top' overlay={<Tooltip id={`tooltip-top`}>Expiry Date</Tooltip>}>
                            <span className={findMonthDifference(row.original.customer_expiry_date) > 0 ? 'text-danger' : 'text-primary'}>{formattedDate !== 'Invalid date' ? formattedDate : ""}</span>
                        </OverlayTrigger>
                    </span>);
            ...
            {
                Header: 'IMEI/Sim/ICCID',
                accessor: 'vehicle_imei',
                className: 'imeicolumn',
                Cell: ({ row }) => {

                    return (
                        <span className="pointer" onClick={() => getVehicle(row.original.vehicle_traccar_id, row.original.customer_traccar_id)}>

                            <span>{row.original.vehicle_imei}<br />{row.original.vehicle_sim}<br />{row.original.vehicle_iccid}</span>

                            <span>
                                {row.original.vehicle_sim_ownership === 'customer' && <OverlayTrigger
                                    placement='top'
                                    overlay={<Tooltip id={`tooltip-top`}>Customer Sim</Tooltip>}>
                                    <Badge variant="primary" style={{ marginLeft: '5px' }}>CS</Badge>
                                </OverlayTrigger>}
                                <OverlayTrigger
                                    placement='top'
                                    overlay={<Tooltip id={`tooltip-top`}>Data</Tooltip>}>
                                    <Badge variant="success" style={{ marginLeft: '5px' }}>{row.original.vehicle_data}</Badge>
                                </OverlayTrigger>
                            </span>
            ...
            },
            {
                ...
                Cell: ({ row }) => {
                    return (<span className="pointer" onClick={() => getVehicle(row.original.vehicle_traccar_id, row.original.customer_traccar_id)}>{row.original.vehicle_installed_date !== '0000-00-00' && row.original.vehicle_installed_date !== null ? Moment(row.original.vehicle_installed_date).format('DD-MM-yyyy') : ''}  </span>);
                }
            },
            {
                Header: 'Service Date',
                ...
                className: 'datecolumn',
                Cell: ({ row }) => {
                    return (<span className="pointer" onClick={() => getVehicle(row.original.vehicle_traccar_id, row.original.customer_traccar_id)}>{row.original.vehicle_services_date !== '0000-00-00' && row.original.vehicle_services_date !== null ? Moment(row.original.vehicle_services_date).format('DD-MM-yyyy') : ''}</span>);
                }
            },
            {
                Header: 'Recent Update',
                ...
                Cell: ({ row }) => {
                    return (<span className="pointer" onClick={() => getVehicle(row.original.vehicle_traccar_id, row.original.customer_traccar_id)}>{Moment(row.original.vehicle_last_updated_time).format('DD-MM-yyyy hh:mm a')}</span>);
                }
            },
            {
                Header: 'NW',
                accessor: 'vehicle_sim_4g',
                className: 'networkcolumn',
                Cell: ({ row }) => {
                    return (<span className="pointer" onClick={() => getVehicle(row.original.vehicle_traccar_id, row.original.customer_traccar_id)}>{row.original.vehicle_sim_4g === 'yes' ? '4G' : row.original.vehicle_sim_4g === 'no' ? '2G' : ''}</span>);
                }
            },
            {
                Header: 'Note',
                accessor: 'note',
                className: 'lastnoteclm',
                Cell: ({ row }) => {
                    ...
                    return (
                        <span>{row.original.inactive === 0 ? <span className="pointer" onClick={() => { showReminderPopup(row.original); }}>
                            <OverlayTrigger
                                placement='top'
                        ...
                        </span> : <span className="pointer" onClick={() => getVehicle(row.original.vehicle_traccar_id, row.original.customer_traccar_id)}>{row.original.inactive_note}</span>}
                        </span>
                    );
                }
            },
    ...
    useEffect(() => {
        if (listupdated) {
            ...
            const remSearchDate = remSearchDtSave
    ...
    const getVehiclesList = useCallback(async ({ pageIndex, searchtext, sortBy, selectedStatus, rectifiedFilter, selectedRemType, remSearchDate, rmode, searchReminder }) => {
        ...
        setRemSearchDtSave(remSearchDate);
        if (sortBy.length > 0) {
                                ...
                                <Card.Body className='task-comment'>
                                    ...
                                    <Form key="vehicleform" onSubmit={handleSubmit(editvehicleform)} >
                                        <Row>
                                            ...
                                            {getValues('inactive') && <Col md={12}>
                                                <Form.Group controlId="exampleForm.ControlSelect1">
                                                    <Form.Label>Note for Duplicate:  </Form.Label>
                                                    <Form.Control as="textarea" rows="2" {...register('inactive_note')} />
                                                </Form.Group>
                                            </Col>}
