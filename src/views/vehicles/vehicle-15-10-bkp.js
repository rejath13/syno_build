...
import { Row, Col, Card, Pagination, Table, Modal, Field, Button, OverlayTrigger, Tooltip, Form, Tabs, Tab, Badge, ButtonGroup, FormControl, InputGroup } from 'react-bootstrap';
...
function App() {
    ...
    const { register: registerEditRemDate, handleSubmit: handleSubmitEditRemDate, reset: resetEditRemDate, getValues: getValuesEditRemDate } = useForm({
        defaultValues: {
            reminder_date: '',
            reminder_id: 0
        },
    ...
    const renderInput = ( props, openCalendar, closeCalendar ) => {
        ...
        return (
            <div>
                <InputGroup>
                    <FormControl type="text" {...props} style={{borderRadius:'0.25rem',padding:'10px'}} />
                    <InputGroup.Append>
                        <InputGroup.Text onClick={openCalendar} style={{cursor: 'pointer',borderRadius:'0.25rem'}}><i className="fa fa-calendar text-primary"/></InputGroup.Text>
                        <InputGroup.Text onClick={closeCalendar} style={{cursor: 'pointer'}}><i className="fa fa-times text-warning"/></InputGroup.Text>
                        <InputGroup.Text onClick={clear} style={{cursor: 'pointer'}}><i className="fa fa-trash-alt text-danger"/></InputGroup.Text>
                    </InputGroup.Append>
                </InputGroup>
    ...
    const editReminderDate = (date) => {
        console.log(new Date(date));
        reset({ ...getValuesEditRemDate(), reminder_date: new Date(date) });
    }

    const submitReminderDate = () => {
        console.log(getValuesEditRemDate());
    }

    const showRmDatePicker = (id, date) => {
        resetEditRemDate({
            reminder_id: id,
            reminder_date: new Date(date)
        });
    }
        ...
        
           {
            ...
            },
            {
                ...
                Cell: ({ row }) => {
                    ...
                    if (getValuesEditRemDate('reminder_id')===0 || getValuesEditRemDate('reminder_id')!==row.original.vehicle_note_id) {
                        return (<span className={getDifferenceInDays(row.original.vehicle_note_date) ? 'redText' : ''} style={{cursor:'pointer'}} onClick={ () => showRmDatePicker(row.original.vehicle_note_id,row.original.vehicle_note_date)}>{row.original.vehicle_note_date !== '0000-00-00 00:00:00' ? Moment(row.original.vehicle_note_date).format('DD-MM-YYYY HH:mm') : ''}</span>);
                    }else{
                        let inputProps = {
                            placeholder: Moment(getValuesEditRemDate('reminder_date')).format('DD-MM-YYYY HH:mm')
                        };

                        return( <Datetime dateFormat="DD-MM-YYYY HH:mm" inputProps={ inputProps } renderInput={ renderInput } /> );
                    }
                }
            },
            {
