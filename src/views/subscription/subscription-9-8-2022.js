...
function App() {
  ...
  const showReminderPopup = () => {
    ...
    const getReminders = useCallback(async () => {
        ...
        try {
            ...
            const url = API_URL + "getSubscriptionReminders";
  ...
  const submitReminderDate = async (data) => {
    ...
    const postdata = { customer_id: selectedCustomerId, subscriber_note: data.subscriber_note.replace(/\r?\n/g, '<br/>'), subscriber_reminder_date: remDate ? Moment(remDate).format('YYYY-MM-DD HH:mm') : '', subscription_note_id:data.subscription_note_id };
                ...
                Cell: ({ row }) => {
                    ...
                    return (
                        <span>
                            ...
                            >
                                <Button onClick={() => showRemDatePopup(row.original.customer_id, row.original.subscriber_reminder_date)} className='text-capitalize' variant="success" style={{ padding: '6px' }}><i className="far fa-sticky-note" style={{ fontWeight: 'normal', margin: 0 }}></i></Button>
                            </OverlayTrigger>
                        </span>
                    );
                                                    ...
                                                    <li className="media">
                                                        ...
                                                        <div className="media-body">
                                                            <h6 className="media-heading text-muted">{note.sub_note_by}
                                                                ...
                                                                <span style={{ fontWeight: 'bold', marginRight: '25px', float: 'right', color: 'black' }}>{note.subscription_note_date!=='0000-00-00 00:00:00' ? '[' + note.subscription_note_date + ']' : ''}</span>
                                        ...
                                        <tbody>
                                            {reminders.map((reminder, index) => (
                                                <tr>
                                                    <td>{index + 1}</td>
                                                    <td>{reminder.name}</td>
                                                    <td><span className={findDatePastorFuture(reminder.subscription_note_date)<0?'redText':''}>{Moment(reminder.subscription_note_date).format('DD-MM-YYYY hh:mm:ss')}</span></td>
